...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Handle Entra block or security group removal (401 for blacklisted tokens, 403 for real-time checks)
    if ((status === 401 || status === 403) && redirect && (reason === 'blocked' || reason === 'security_group')) {
